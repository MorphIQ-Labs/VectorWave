 ...
 * }</pre>
 * 
 * <h4>Timeout-Aware Processing:</h4>
 * <pre>{@code
 * try {
 ...
 */
public class StructuredParallelTransform implements AutoCloseable {
    ...
    public StructuredParallelTransform(Wavelet wavelet, BoundaryMode boundaryMode) {
        ...
        this.config = ParallelConfig.auto();
    ...
    /**
     * Closes this transform and releases any owned resources. If this instance
     * created its own ParallelConfig (default constructor), the associated
     * executors are shut down. If a ParallelConfig was provided by the caller,
     * it remains the caller's responsibility to shut it down.
     */
    @Override
    public void close() {
        if (ownsConfig) {
            try { config.shutdown(); } catch (Throwable ignored) {}
        }
    }
}
