...
import com.morphiqlabs.wavelet.extensions.cwt.optimization.CWTVectorOps;
import com.morphiqlabs.wavelet.extensions.util.OptimizedFFT;
...
public class ParallelCWTTransform implements AutoCloseable {
    ...
    private static final System.Logger LOG = com.morphiqlabs.wavelet.util.Logging.getLogger(ParallelCWTTransform.class);
     ...
     * @return continuous wavelet transform result
     */
            ...
            synchronized (shutdownLock) {
                if (!shutdownHookRegistered) {
                    Runtime.getRuntime().addShutdownHook(new Thread(() -> {
                        if (!closed) {
                            LOG.log(System.Logger.Level.WARNING, "ParallelCWTTransform: JVM shutdown detected, cleaning up resources...");
    ...
    private void doClose() {
        synchronized (shutdownLock) {
            ...
            try {
                ...
                LOG.log(System.Logger.Level.WARNING, "Error during ParallelCWTTransform shutdown: {0}", e.getMessage());
            }
        }
    }
    ...
    private void shutdownExecutorGracefully(ExecutorService executor, String name) {
        if (executor != null && !executor.isShutdown() && !isCommonPool(executor)) {
            executor.shutdown();
            LOG.log(System.Logger.Level.DEBUG, () -> "Initiated graceful shutdown of " + name);
        } else if (isCommonPool(executor)) {
            LOG.log(System.Logger.Level.DEBUG, () -> "Skipping shutdown of " + name + " (using common pool)");
        }
    }
    ...
    private boolean awaitTermination(ExecutorService executor, long timeoutSeconds, String name) {
        try {
            ...
            if (terminated) {
                final String n = name;
                LOG.log(System.Logger.Level.DEBUG, () -> n + " terminated gracefully");
            } else {
                LOG.log(System.Logger.Level.WARNING, () -> "" + name + " did not terminate within " + timeoutSeconds + " seconds");
            }
            return terminated;
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            LOG.log(System.Logger.Level.WARNING, () -> "Interrupted while waiting for " + name + " termination");
            return false;
        }
    }
    ...
    private void forceShutdown(ExecutorService executor, String name) {
        try {
            ...
            if (terminated) {
                LOG.log(System.Logger.Level.DEBUG, () -> name + " terminated after forced shutdown");
            } else {
                LOG.log(System.Logger.Level.WARNING, () -> name + " did not terminate even after forced shutdown");
            }
        } catch (InterruptedException e) {
            Thread.currentThread().interrupt();
            LOG.log(System.Logger.Level.WARNING, () -> "Interrupted during forced shutdown of " + name);
        }
    }
    ...
    public static class Builder {
         ...
         * @return new parallel CWT transform
         */
