...
public class OptimizerRegistry {
    private static final Logger LOGGER = Logger.getLogger(OptimizerRegistry.class.getName());
    ...
    private synchronized void initialize() {
        ...
        LOGGER.info("Initializing optimizer registry...");
        ...
        initialized = true;
        LOGGER.info("Optimizer registry initialized with " + optimizerCache.size() + " optimizers");
    }
    ...
    private <T> void discoverOptimizers(Class<T> optimizerClass) {
        ...
        for (T optimizer : loader) {
            try {
                if (isSupported(optimizer)) {
                    ...
                    LOGGER.info("Found " + optimizerClass.getSimpleName() + ": " + 
                              getName(optimizer) + " (priority: " + getPriority(optimizer) + ")");
                }
            } catch (Exception e) {
                LOGGER.log(Level.WARNING, "Error checking optimizer support: " + optimizer, e);
            }
        }
        ...
        if (!candidates.isEmpty()) {
            ...
            LOGGER.info("Selected " + optimizerClass.getSimpleName() + ": " + getName(best));
        } else {
            LOGGER.info("No " + optimizerClass.getSimpleName() + " optimizers found, will use scalar fallback");
        }
    }
